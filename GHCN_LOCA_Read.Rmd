 ...
 # Plot all the ensembles for one case
  ...
  ggplot(data = all_year)  +
    ...
    labs(title = paste(loca_year[,]$Station,
                       ...
                       " CMIP5 LOCA vs Obs",
                       sep = ""),
         y     = paste('Total Annual Rainfall (mm)',
                       sep = "") )
...
Let's take a closer look at the precipitation and temprature rates of change over time...

We can add a trend line to the data and model
...
```{r}
  ...
  ggplot(data = all_year)  +
    ...
    scale_alpha_continuous(range = c(0.3, 1.0),
                           guide = FALSE)  + 
    scale_color_manual(values = c("blue", "red", "black")) + 
    scale_fill_manual( values = c("cyan", "magenta", "grey")) + 
    theme_bw() + 
    theme(plot.title = element_text(hjust = 0.5)) +
    annotate("text", 
             x=min(all_year$Time), 
...
```{r}
  ...
  ggplot(data = all_year)  +
    ...
    labs(title = paste(loca_year[,]$Station,
                       ...
                       " CMIP5 LOCA vs Obs",
                       sep = ""),
         y     = paste('Total Precipitation (mm)',
                       sep = "") )
...
```{r}
 ...
 # Plot all the ensembles for one case
  ...
  mysubset = subset(all_year_period_1, Scenario!="RCP 8.5")
...
ggplot(data=subset(all_year,  !is.na(Case)   ))  +
...
ggplot(data=subset(all_daily,  !is.na(Case) & !is.na(Days_Between_Wetting_Rain)   ))  +
